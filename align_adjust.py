...
def cleanAli(recordNuc):
    ...
    if args.ign == False and args.omit == False:
        records = adjust(records)

    with open(args.o, 'w') as fp:
        SeqIO.write(records, fp, args.otype)


def findStop(records):
    for rec in records:
        pos = [i for i, x in enumerate(spliter(rec.seq, 3)) if 'TAA' in x or 'TAG' in x or 'TGA' in x]
        if pos != []:
            break
...
def countStop(records):
    stopPos = list()
    for rec in records:
        sequence = spliter(rec.seq, 3)
        for i, nucs in enumerate(sequence):
            if 'TAA' in nucs or 'TAG' in nucs or 'TGA' in nucs:
                stopPos.append(i)

    return(len(set(stopPos)))


def adjust(records):
    totalStopPos = countStop(records)
    
    counter = 1
    while counter <= totalStopPos:
        pos = findStop(records)[0]
        for i, rec in enumerate(records):
            if 'TAA' in rec.seq[pos*3:pos*3+3] or 'TAG' in rec.seq[pos*3:pos*3+3] or 'TGA' in rec.seq[pos*3:pos*3+3]:
                records[i].seq = rec.seq[:pos*3] + Seq("NNN", generic_dna) + rec.seq[pos*3+3:]

            else:
                records[i].seq = rec.seq[:pos*3] + Seq("---", generic_dna) + rec.seq[pos*3:-3]
